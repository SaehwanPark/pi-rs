...
/// Shared with `pi-rs run --resume`, which names a session the same way and must not
/// reach a different verdict than the reader would. The `None` default is the trace
/// command's only, which is why it speaks the trace prefix.
