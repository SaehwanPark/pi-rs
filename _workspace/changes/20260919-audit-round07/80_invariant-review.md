...
- `crates/rupi-provider/src/decode.rs:193` rejects duplicate provider call IDs in one
  decoded response before emitting any call; IDs may be reused in later responses.
- `crates/rupi-runtime/src/turn.rs:2731` parents normal tool lifecycle events as
  completion -> request -> start -> terminal outcome. `crates/rupi-store/src/store.rs:3722`
  resolves by causal event identity and only falls back to a unique active provider ID
  for parentless legacy traces.
- `crates/rupi-runtime/src/turn.rs:709` uses typed `Refused` management outcomes;
...
- `crates/rupi-store/src/store.rs:703` preflights the redacted semantic record before
...
- `crates/rupi-provider/src/openai.rs:47` uses the shared URL redactor, and
  `crates/rupi-provider/src/relay.rs:35` bounds detached resolver work to one worker
...
- `cargo check -p rupi --target x86_64-pc-windows-msvc`
