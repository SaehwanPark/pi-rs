---
name: rupi-change-orchestrator
description: Plan and deliver non-trivial rupi changes as bounded roadmap-aligned slices with explicit routing, ownership, review, and verification.
---

# rupi Change Orchestrator

## When to Use

Use this skill for a non-trivial feature, refactor, compatibility change, or runtime behavior change in `rupi`.
...
- Use `rupi-core-contract-designer` when the change defines events, identities, provider failures, tool lifecycle, provenance, model epochs, sessions, traces, context policies/capsules, failover state, trust, or redaction contracts.
- Use `rupi-compatibility-fixture-author` when behavior affects Pi skills, prompts, packages, sessions, themes, or extension APIs.
- Independently require `rupi-invariant-reviewer` after non-trivial runtime changes or changes on startup, persistence, failover, context, or security paths.
...
See `../../../docs/harness/rupi-development/team-spec.md` for routing, handoff, and failure policy.
