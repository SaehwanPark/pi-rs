...
//! whenever a user runs `pi-rs run --resume <id>` or launches the interactive surface
...
//!
//! The benchmark measures `pi_rs_store::session_log::restore`, which parses the durable
//! JSONL log, tracks sequence continuity, records checkpoint barriers, and restores
//! active message state.
//!
//! Run: `cargo bench -p pi-rs-store --bench restore`
...
use pi_rs_store::{SessionLog, restore};
...
fn main() -> ExitCode {
  ...
  println!("Session restore benchmark (pi-rs-store):");
...
fn create_session_file(
  ...
  for turn_idx in 1..=turns {
    ...
    let assistant_msg = SessionRecord::Message(SessionMessage {
      turn_id: TurnId::new(),
      role: pi_rs_core::message::Role::Assistant,
