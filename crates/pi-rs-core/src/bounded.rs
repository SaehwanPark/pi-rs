...
/// [`LineOverflow::Reject`], no bytes beyond the bounded prefix are consumed on
/// overflow; this is intentional for protocol callers, which should tear down
/// the transport instead of trying to discard an arbitrary remainder.
///
/// With [`LineOverflow::Truncate`], the remainder is consumed without being
/// retained so a local text scan can continue to the next line. In both modes,
/// EOF after a partial line returns that partial line.
  ...
  loop {
    ...
    if buffer.is_empty() {
      return if line.is_empty() {
        ...
        Ok(Some(BoundedLine::Truncated(line)))
      } else {
    ...
    reader.consume(take);
    if newline.is_some() {
      return Ok(Some(BoundedLine::Truncated(line)));
    }
  }
}
