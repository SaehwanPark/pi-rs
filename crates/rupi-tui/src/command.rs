...
mod tests {
  ...
  fn path_like_slashes_are_prompts_not_commands() {
    let input = Input::parse("/home/saehwan/repos/rupi");
    assert!(!input.is_command());
    assert!(
      input
        .spans()
        .iter()
        .any(|s| s.role == Role::Path && s.text == "/home/saehwan/repos/rupi"),
      "{:?}",
      input.spans()
    );
  }
  ...
  fn command_arguments_keep_paths_and_flags_apart() {
    let input = Input::parse("/run --config cfg.json crates/rupi-core/src/lib.rs");
