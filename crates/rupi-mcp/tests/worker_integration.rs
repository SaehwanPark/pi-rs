...
use rupi_core::{ModelRef, now_millis};
use rupi_mcp::protocol::{CallToolResult, JsonRpcRequest};
use rupi_mcp::{
...
impl WorkerEngine for FixtureEngine {
  fn run(
    ...
    cancel: rupi_mcp::WorkerCancelToken,
  ) -> Result<WorkerExecution, rupi_mcp::WorkerError> {
...
fn public_worker_boundary_supports_run_cancel_and_resource_reads() {
  ...
  let handle = service
    ...
    .expect("start");
  assert_eq!(handle.status, rupi_mcp::WorkerStatus::Completed);
  let summary = service
  ...
  let cancelled = service
    .cancel(rupi_mcp::CancelRequest {
      session_id: running.session_id,
      run_id: running.run_id,
      reason: Some("orchestrator stop".into()),
    })
    .expect("cancel");
  assert_eq!(cancelled.status, rupi_mcp::WorkerStatus::Cancelling);
}
...
fn cancel_token_is_atomic_and_cloneable() {
  let token = rupi_mcp::WorkerCancelToken::default();
