...
use fs4::FileExt;
use rupi_core::ids::uuidv7;

use crate::StoreError;

const OWNER_FILE: &str = "owner";
const LOCK_FILE: &str = "lock";
const LOCK_MARKER: &str = "rupi-lock-v1";
...
impl SessionLease {
  pub fn acquire(path: &Path) -> Result<Self, StoreError> {
    ...
    // A directory written by an older rupi process has an owner marker but no
...
mod tests {
  ...
  fn lock_is_released_when_owner_process_is_killed() {
    let child_path = std::env::var_os("RUPI_LEASE_CHILD");
