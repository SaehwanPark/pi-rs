...
//!
//! This crate deliberately depends only on the generic rupi MCP and context
...
use rupi_mcp::{CallToolResult, McpError, McpManager, McpTool, McpTransport};
...
impl RkbReference {
  ...
  /// Convert to rupi' provider-neutral durable reference contract.
...
impl RkbConnection {
  ...
  fn call(&self, tool: &McpTool, arguments: Value) -> Result<String, RkbError> {
    ...
    impl ToolProgress for NoProgress {
      fn emit(&mut self, _chunk: &rupi_core::ToolChunk) {}
    }
