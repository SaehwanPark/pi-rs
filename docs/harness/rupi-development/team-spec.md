# rupi Development Harness

## Domain Summary

`rupi` is currently at its repository-and-contract foundation. Its highest-value reusable workflow is not broad autonomous implementation; it is disciplined delivery of small runtime slices while protecting explicit state, provenance, recovery, compatibility, and startup boundaries.
...
| --- | --- | --- | --- |
| Change owner | Bounds, synthesizes, verifies, and accepts the slice; implements unless an assigned specialist owns paths | `.agents/skills/rupi-change-orchestrator/SKILL.md` | unassigned source/tests/docs and optional `00`, `90` artifacts |
| Contract designer | Defines typed runtime states and failure semantics; advises read-only or becomes sole writer for assigned paths | `.agents/skills/rupi-core-contract-designer/SKILL.md` | assigned contract code/tests and optional `10_contract-notes.md` |
| Compatibility author | Captures Pi-facing behavior in fixtures and diagnostics; advises read-only or becomes sole writer for assigned paths | `.agents/skills/rupi-compatibility-fixture-author/SKILL.md` | assigned compatibility code/tests and optional `20_compatibility-report.md` |
| Invariant reviewer | Performs read-only architecture-sensitive review | `.agents/skills/rupi-invariant-reviewer/SKILL.md` | optional `80_invariant-review.md` only |
