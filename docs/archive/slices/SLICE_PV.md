...
Verified by grep, do not re-derive: `ReasoningProvenance` is `crates/rupi-core/src/provenance.rs:25`,
...
Test-only. Prefer `crates/rupi-core/tests/` (new file) unless the crate's convention puts these in-unit —
check what exists first with `ls crates/rupi-core/tests/ | head` and say which you chose and why.
≤3 `read` calls at ≤40 lines; outputs ≤20 lines; `cargo test -p rupi-core` only, `--workspace` once at the
