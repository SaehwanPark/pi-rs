...
- Projection API (`crates/rupi-tui/src/statusline.rs`): `Status { model, activity, turns, columns, hint }`,
...
3. `crates/rupi-tui/src/statusline.rs` with `offset=1, limit=70` — field names and `Activity` variants.
4. To find a turn counter: `grep -n "turns\|TurnState::Working" src/interactive.rs | head -12`.

## The change
1. Rewrite `status_line` so its body builds `rupi_tui::statusline::Status { .. }` and returns the
...
3. **The loop's no-spill invariant wins.** The projection's floor at `columns: 0` is `"idle"`
   ...
   than `columns`, truncate with `rupi_tui::width::truncate`. Keep the existing test's meaning:
