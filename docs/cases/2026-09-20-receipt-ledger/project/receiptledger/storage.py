"""SQLite state and token-fenced pipeline/job transitions."""
...
from .ids import content_hash, delivery_key, normalize_pipeline
...
class Store:
        ...
        with self._session() as connection:
            ...
            jobs = connection.execute(
                ...
                SELECT job_id, kind, payload, depends_on, input_refs, collect, status, attempts,
                       last_error, lease_expires_at, output, receipt
        ...
        with self._session() as connection:
            ...
            for row in rows:
                ...
                if job["collect"] is not None:
                    try:
                        fan_in = self._resolve_collect(
                            connection, job["pipeline_id"], dependencies, job["collect"]
                        )
                ...
                updated = connection.execute(
                    """
                    UPDATE jobs SET status = 'leased', lease_expires_at = ?, lease_token = ?,
                        attempts = attempts + 1
                    WHERE pipeline_id = ? AND job_id = ? AND status = 'pending'
                    """,
                    (expires, token, job["pipeline_id"], job["job_id"]),
                ).rowcount
                if updated != 1:
                    continue
    ...
    def finish_success(self, job: dict[str, Any], output: dict[str, Any], receipt_id: str) -> bool:
        receipt = {"delivery_key": job["delivery_key"], "receipt_id": receipt_id}
        return self._finish(job, "succeeded", None, output, receipt, "observed_success", "sink_receipt")

    def finish_retryable(
        self, job: dict[str, Any], message: str, outcome: str = "observed_failure"
    ) -> bool:
        return self._finish(job, "pending", message, None, None, outcome, "retryable_failure")

    def finish_terminal(self, job: dict[str, Any], message: str) -> bool:
        return self._finish(job, "failed", message, None, None, "observed_failure", "terminal_failure")
        ...
        with self._session() as connection:
            ...
            updated = connection.execute(
                ...
                UPDATE jobs SET status = ?, last_error = ?, lease_expires_at = NULL,
                    ...
                    receipt = CASE WHEN ? = 'succeeded' THEN ? ELSE receipt END
                WHERE pipeline_id = ? AND job_id = ? AND status = 'leased' AND lease_token = ?
                """,
                (
                    status,
                    message,
                    ...
                    status,
                    _dump(receipt) if receipt is not None else None,
                    job["pipeline_id"],
                    job["job_id"],
                    job["lease_token"],
                ),
    ...
    def _refresh(self, connection: sqlite3.Connection) -> None:
        expired = connection.execute(
            """
            SELECT pipeline_id, job_id, attempts FROM jobs
        ...
        for row in expired:
            changed = connection.execute(
                """
                UPDATE jobs SET status = 'pending', lease_expires_at = NULL, lease_token = NULL,
            ...
            if changed == 1:
                append_event(
                    ...
                    job_id=row["job_id"],
                    delivery_key=delivery_key(row["pipeline_id"], row["job_id"]),
                    status="pending",
                    attempts=int(row["attempts"]),
                    outcome="unknown",
        ...
        while changed:
            changed = False
            rows = connection.execute(
                "SELECT pipeline_id, job_id, depends_on, attempts FROM jobs WHERE status = 'pending'"
            ).fetchall()
            for row in rows:
                dependencies = json.loads(row["depends_on"])
                if not dependencies:
                ...
                if any(statuses.get(dependency) in {"failed", "blocked"} for dependency in dependencies):
                    updated = connection.execute(
                        """
                        UPDATE jobs SET status = 'blocked', lease_expires_at = NULL, lease_token = NULL,
            ...
            "jobs": [
                {
                    ...
                    "delivery_key": delivery_key(pipeline_id, job["job_id"]),
                    "status": job["status"],
                    "attempts": int(job["attempts"]),
                    "last_error": job["last_error"],
                    "lease_expires_at": job["lease_expires_at"],
                    "output": json.loads(job["output"]) if job["output"] is not None else None,
                    "receipt": json.loads(job["receipt"]) if job["receipt"] is not None else None,
                }
                for job in jobs
            ],
        }
