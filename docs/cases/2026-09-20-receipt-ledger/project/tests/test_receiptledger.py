...
from receiptledger.audit import canonical_event, verify_database
from receiptledger.ids import ValidationError, delivery_key, normalize_pipeline
from receiptledger.storage import Store
from receiptledger.worker import _invoke_sink
...
class ReceiptLedgerTests(unittest.TestCase):
    ...
    def test_admission_is_idempotent_conflict_safe_and_audited(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            count, _head = verify_database(database)
            self.assertEqual(count, 1)
            with closing(sqlite3.connect(database)) as connection:
                connection.execute("BEGIN")
                event = json.loads(connection.execute("SELECT event_json FROM audit_events").fetchone()[0])
                connection.commit()
            self.assertEqual(event["kind"], "pipeline_admitted")

    def test_claim_success_persists_output_receipt_and_audit(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            database = Path(directory) / "state.sqlite3"
            store = Store(database)
            ...
            self.assertEqual(result["jobs"][0]["receipt"]["receipt_id"], "receipt-finish-source")
            count, _head = verify_database(database)
    ...
    def test_stale_token_cannot_change_newer_receipt(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            stale = dict(first)
            self.assertTrue(store.finish_success(first, {"worker": "winner"}, "receipt-fence-source"))
            ...
            with closing(sqlite3.connect(database)) as connection:
                kinds = [row[0] for row in connection.execute("SELECT event_json FROM audit_events")]
            self.assertIn("stale_finalization_rejected", [json.loads(item)["kind"] for item in kinds])

    def test_expired_claim_is_reclaimed_and_chain_remains_valid(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            database = Path(directory) / "state.sqlite3"
            store = Store(database)
            store.admit(one_job("reclaim"))
            store.claim_next(set(), 1)
            time.sleep(1.05)
    ...
    def test_declared_inputs_and_barrier_order(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(join["fan_in"]["items"], [
                {"job_id": "a", "value": "a-out"},
                {"job_id": "b", "value": "b-out"},
            ])

    def test_sink_response_requires_matching_delivery_receipt(self) -> None:
        request_value = {"pipeline_id": "p", "job_id": "source", "delivery_key": "p:source"}
        code = (
        ...
        outcome, detail = _invoke_sink(sys.executable, ["-c", code], request_value, "source", "p:source")
        ...
        outcome, detail = _invoke_sink(sys.executable, ["-c", mismatch_code], request_value, "source", "p:source")
        self.assertEqual(outcome, "retryable")
        self.assertEqual(detail["audit_outcome"], "unknown")

    def test_hash_formula_is_canonical(self) -> None:
        event = {"kind": "pipeline_admitted", "seq": 1}
        event_json = canonical_event(event)
        self.assertEqual(
            hashlib.sha256(("0" * 64 + "\n" + event_json).encode("utf-8")).hexdigest(),
            hashlib.sha256(("0" * 64 + "\n" + '{"kind":"pipeline_admitted","seq":1}').encode("utf-8")).hexdigest(),
        )

    def test_cli_help_is_available(self) -> None:
        for arguments in (["--help"], ["serve", "--help"], ["worker", "--help"], ["audit", "--help"]):
