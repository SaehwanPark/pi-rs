...
def canonical_event(value: dict) -> str:
...
def worker_command(
    ...
    *,
    ...
    if lose_ack_job is not None:
        sink_args.extend(["--lose-ack-job", lose_ack_job])
    command = [
        sys.executable,
        "-m",
        "receiptledger",
...
def run_audit(db: Path, *arguments: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "receiptledger", "audit", "--db", str(db), *arguments],
        cwd=PROJECT_ROOT,
        capture_output=True,
        text=True,
        check=False,
...
def assert_audit_chain(test: unittest.TestCase, db: Path) -> list[dict]:
    rows = read_audit_rows(db)
    previous = ZERO_HASH
    events: list[dict] = []
    for expected_seq, (seq, event_json, prev_hash, event_hash) in enumerate(rows, 1):
        test.assertEqual(seq, expected_seq)
        test.assertEqual(prev_hash, previous)
        event = json.loads(event_json)
        test.assertEqual(event["seq"], seq)
        test.assertEqual(event_json, canonical_event(event))
        expected_hash = hashlib.sha256((prev_hash + "\n" + event_json).encode("utf-8")).hexdigest()
        test.assertEqual(event_hash, expected_hash)
        events.append(event)
        previous = event_hash
    serialized = json.dumps(events, sort_keys=True)
    for forbidden in (SECRET, "claim_token", "lease_token", "private_token", "--sink-arg"):
        test.assertNotIn(forbidden, serialized)
    return events


class ReceiptLedgerOracle(unittest.TestCase):
    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory(prefix="receipt-ledger-oracle-")
    ...
    def test_audit_chain_data_flow_restart_and_tamper(self) -> None:
        base = self.start()
        ...
        code, _stdout, stderr = run_worker(worker_command(self.db, "ordered-worker", log, receipts))
        self.assertEqual(code, 0, stderr)
        entries = read_log(log)
        self.assertEqual(
            [entry["job_id"] for entry in entries],
            ["source", "part-a", "part-b", "barrier", "publish"],
        )
        ...
        self.assertEqual(
            [item["value"] for item in barrier["fan_in"]["items"]],
            ["artifact-ordered-part-a", "artifact-ordered-part-b"],
        )
        publish = next(entry for entry in entries if entry["job_id"] == "publish")
        self.assertEqual(
            publish["inputs"],
            {"joined": ["artifact-ordered-part-a", "artifact-ordered-part-b"]},
        )
        ...
        audit = run_audit(self.db, "--verify")
        self.assertEqual(audit.returncode, 0, audit.stderr)
        tail = run_audit(self.db, "--tail", "4")
        self.assertEqual(tail.returncode, 0, tail.stderr)
        self.assertGreaterEqual(len([line for line in tail.stdout.splitlines() if line.strip()]), 1)
        events = assert_audit_chain(self, self.db)
        kinds = [event["kind"] for event in events]
        self.assertEqual(kinds[0], "pipeline_admitted")
        self.assertEqual(kinds.count("pipeline_admitted"), 1)
        self.assertEqual(kinds.count("job_claimed"), 5)
        self.assertEqual(kinds.count("job_succeeded"), 5)
        ...
        audit = run_audit(self.db, "--verify")
        self.assertEqual(audit.returncode, 0, audit.stderr)
        assert_audit_chain(self, self.db)
        ...
        with sqlite3.connect(self.db) as connection:
            ...
            connection.commit()
        tampered = run_audit(self.db, "--verify")
        self.assertNotEqual(tampered.returncode, 0)
        self.assertIn("audit", (tampered.stderr + tampered.stdout).lower())

    def test_retry_terminal_blocked_and_audit(self) -> None:
        base = self.start()
        ...
        self.assertEqual(audit.returncode, 0, audit.stderr)
        events = assert_audit_chain(self, self.db)
        self.assertIn("job_retryable_failure", [event["kind"] for event in events])
        self.assertIn("job_failed", [event["kind"] for event in events])
        self.assertIn("job_blocked", [event["kind"] for event in events])

    def test_fencing_and_lost_ack_receipt_are_audited(self) -> None:
        base = self.start()
        ...
        try:
            ...
            self.assertEqual(code, 0, stderr)
            winner = http_call(base, "GET", "/pipelines/fenced")[1]
            ...
            unchanged = http_call(base, "GET", "/pipelines/fenced")[1]
            self.assertEqual(unchanged, winner)
            self.assertEqual(read_log(first_log)[0]["delivery_key"], read_log(second_log)[0]["delivery_key"])
        finally:
        ...
        try:
            ...
            code, _stdout, stderr = run_worker(
                worker_command(self.db, "recovery-b", second_log, receipt_store, lease_seconds=1)
            )
            self.assertEqual(code, 0, stderr)
            result = http_call(base, "GET", "/pipelines/receipt")[1]
            self.assertEqual(result["status"], "succeeded")
            job = result["jobs"][0]
            ...
            events = assert_audit_chain(self, self.db)
            kinds = [event["kind"] for event in events]
            self.assertGreaterEqual(kinds.count("lease_reclaimed"), 1)
            self.assertEqual(kinds.count("job_succeeded"), 2)
            audit = run_audit(self.db, "--verify")
            self.assertEqual(audit.returncode, 0, audit.stderr)
        finally:
            if pid_file.exists():
                stop_helper_process(int(pid_file.read_text(encoding="utf-8").strip()))
            release.write_text("release\n", encoding="utf-8")
            if first.poll() is None:
                first.kill()
    ...
    def test_help_and_standard_library_boundary(self) -> None:
        for arguments in (
            ...
            completed = subprocess.run(
                [sys.executable, "-m", "receiptledger", *arguments],
                cwd=PROJECT_ROOT,
                capture_output=True,
                text=True,
                check=False,
        ...
        allowed = set(getattr(sys, "stdlib_module_names", ())) | {"receiptledger"}
        forbidden: list[str] = []
