...
class BatchRelayOracle(unittest.TestCase):
    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory(prefix="batch-relay-oracle-")
        self.root = Path(self.temp.name)
        self.db = self.root / "state" / "relay.sqlite3"
        self.port = free_port()
    ...
    def post_batch(self, batch: dict, expected: int | None = None):
        body, signature = signed(batch)
        result = http_call(self.base, "POST", "/batches", body, signature)
        if expected is not None:
            self.assertEqual(result[0], expected, result)
        return result

    def get_batch(self, batch_id: str) -> dict:
        status, data = http_call(self.base, "GET", f"/batches/{batch_id}")
    ...
    def test_signed_atomic_admission_dependency_order_and_restart(self) -> None:
        invalid = {
            "batch_id": "cycle",
            "jobs": [
                {"job_id": "a", "kind": "a", "payload": {}, "depends_on": ["b"]},
                {"job_id": "b", "kind": "b", "payload": {}, "depends_on": ["a"]},
            ],
        }
        body, _ = signed(invalid)
        status, data = http_call(self.base, "POST", "/batches", body, "sha256=bad")
        self.assertEqual(status, 401)
        self.assertIn("error", data)
        status, data = self.post_batch(invalid)
        self.assertEqual(status, 400)
        self.assertIn("error", data)
        self.assertEqual(http_call(self.base, "GET", "/batches/cycle")[0], 404)

        batch = {
            "batch_id": "release-1",
            "jobs": [
                {"job_id": "build", "kind": "build", "payload": {"v": 5}, "depends_on": []},
                {"job_id": "publish", "kind": "publish", "payload": {"channel": "stable"}, "depends_on": ["build"]},
            ],
        }
        first = self.post_batch(batch, 202)
        self.assertEqual(first[1]["status"], "pending")
        self.assertEqual(self.post_batch(batch, 200)[1], first[1])
        conflict = dict(batch, jobs=[dict(batch["jobs"][0], payload={"v": 6}), batch["jobs"][1]])
        self.assertEqual(self.post_batch(conflict, 409)[0], 409)

        log = self.root / "ordered.log"
        result = self.run_worker(log)
        self.assertEqual(result.returncode, 0, result.stderr)
        delivered = [json.loads(line)["job_id"] for line in log.read_text(encoding="utf-8").splitlines()]
        self.assertEqual(delivered, ["build", "publish"])
        state = self.get_batch("release-1")
    ...
    def test_retryable_and_terminal_failures(self) -> None:
        retry_batch = {
            "batch_id": "retry-1",
            "jobs": [
                {"job_id": "fetch", "kind": "fetch", "payload": {}, "depends_on": []},
                {"job_id": "store", "kind": "store", "payload": {}, "depends_on": ["fetch"]},
            ],
        }
        self.post_batch(retry_batch, 202)
        log = self.root / "retry.log"
        marker = self.root / "retry.marker"
        result = self.run_worker(log, "--retry-job", "fetch", "--retry-marker", str(marker))
        self.assertNotEqual(result.returncode, 0)
        state = self.get_batch("retry-1")
        ...
        result = self.run_worker(log, "--retry-job", "fetch", "--retry-marker", str(marker))
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(self.get_batch("retry-1")["status"], "succeeded")

        failed = {
            "batch_id": "failed-1",
            "jobs": [
                {"job_id": "root", "kind": "root", "payload": {}, "depends_on": []},
                {"job_id": "child", "kind": "child", "payload": {}, "depends_on": ["root"]},
            ],
        }
        self.post_batch(failed, 202)
    ...
    def test_crashed_lease_is_reclaimed_by_fresh_worker(self) -> None:
        batch = {
            "batch_id": "crash-1",
            "jobs": [{"job_id": "only", "kind": "work", "payload": {}, "depends_on": []}],
        }
        self.post_batch(batch, 202)
        ...
        result = self.run_worker(log)
        self.assertEqual(result.returncode, 0, result.stderr)
        job = self.get_batch("crash-1")["jobs"][0]
    ...
    def test_help_commands(self) -> None:
        for args in ([], ["serve"], ["worker"]):
            result = subprocess.run(
                [sys.executable, "-m", "batchrelay", *args, "--help"],
                cwd=PROJECT_ROOT,
                capture_output=True,
                text=True,
                timeout=5,
