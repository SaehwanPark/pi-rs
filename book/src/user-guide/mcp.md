...
`rupi` includes a built-in client for the **Model Context Protocol (MCP)**, allowing agents to discover and invoke tools provided by external processes. Stdio and bounded Streamable HTTP POST transports are supported; long-lived server push remains outside this release.

---

## MCP Design in rupi: Lazy Stdio Client

Many agent runtimes eagerly spin up every declared MCP server at process launch, which degrades cold startup times.

`rupi` keeps MCP activation lazy:
...
Tools exposed by MCP servers are automatically mapped into `rupi` tool definitions with distinct namespaces (e.g. `sqlite:query`).
