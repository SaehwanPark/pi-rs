"""SQLite persistence and token-fenced pipeline/job transitions."""
...
class Store:
    ...
    def finish_success(self, job: dict[str, Any], output: dict[str, Any], receipt_id: str) -> bool:
        ...
        return self._finish(job, "succeeded", None, output, receipt)

    def finish_retryable(self, job: dict[str, Any], message: str) -> bool:
        return self._finish(job, "pending", message, None, None)

    def finish_terminal(self, job: dict[str, Any], message: str) -> bool:
        return self._finish(job, "failed", message, None, None)
    ...
    def _resolve_collect(
        ...
        for dependency in dependencies:
            ...
            if not isinstance(output, dict) or collect["field"] not in output:
                raise InputResolutionError(f"collect {collect['as']} is missing field {collect['field']} from {dependency}")
