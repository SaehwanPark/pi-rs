...
def append_log(path: Path, request: dict) -> None:
...
def main() -> int:
    ...
    if not isinstance(request, dict) or not isinstance(request.get("job_id"), str):
        return 2
    append_log(Path(args.log), request)
    ...
    if kind == "barrier":
        fan_in = request.get("fan_in")
        if not isinstance(fan_in, dict):
            return 2
        items = fan_in.get("items")
        if not isinstance(items, list):
            return 2
        output["joined"] = [item["value"] for item in items]
        output["joined_ids"] = [item["job_id"] for item in items]
    elif kind == "publish":
        output["used"] = request.get("inputs", {}).get("joined")
