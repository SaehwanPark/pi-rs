"""Bounded direct-argv worker with stale-claim fencing."""
...
def run(db: str, sink: str, sink_args: list[str], lease_seconds: int) -> int:
    ...
    while True:
        ...
        outcome, detail = _invoke_sink(sink, sink_args, sink_request, job["job_id"])
        if outcome == "success":
            accepted = store.finish_success(job, detail)
