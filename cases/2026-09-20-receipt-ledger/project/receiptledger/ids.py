"""Validation and canonicalization for the Receipt Ledger request shape."""
    ...
    for index, raw_job in enumerate(jobs_value):
        ...
        for dependency in dependencies:
            ...
            if normalized == job_id:
                raise ValidationError(f"job {job_id} cannot depend on itself")
        ...
        for input_name, reference in job["input_refs"].items():
            ...
            if source_job not in job_ids:
                raise ValidationError(
                    f"job {job['job_id']} input {input_name} references missing job: {source_job}"
                )
...
def delivery_key(pipeline_id: str, job_id: str) -> str:
