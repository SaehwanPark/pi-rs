"""Validation and canonicalization for the Artifact Pipeline request shape."""
...
JOB_FIELDS = {"job_id", "kind", "payload", "depends_on", "input_refs"}
...
def normalize_pipeline(document: Any) -> dict[str, Any]:
    ...
    for index, raw_job in enumerate(jobs_value):
        if not isinstance(raw_job, dict) or set(raw_job) != JOB_FIELDS:
            raise ValidationError(f"job {index} must contain exactly the five documented fields")
        ...
        for input_name, raw_ref in input_refs.items():
            ...
            if not isinstance(field, str) or not field.strip() or "." in field:
                raise ValidationError(f"job {job_id} input reference {normalized_name} has an invalid field")
            normalized_refs[normalized_name] = {"job_id": source_job, "field": field.strip()}
